    ...
    try {
      ...
      if (!username) {
        return res.status(400).send({
          message:
            "Não foi possível criar o usuário. O username não foi encontrado!",
          hint: "Por favor, defina um username para o usuário!",
        });
      } else if (!password) {
        return res.status(400).send({
          message:
            "Não foi possível criar o usuário. A senha não foi encontrada!",
          hint: "Por favor, defina uma senha para o usuário!",
        });
      } else if (!email) {
        return res.status(400).send({
          message:
            "Não foi possível criar o usuário. O email não foi encontrado!",
        ...
        if (err) {
          if (err.name == "Already in use") {
            return res.status(409).send({
              message:
                "Não foi possível criar o usuário. O email já está em uso!",
              details: err.message,
  ...
  async loginUserHandler(req: Request, res: Response) {
    ...
    try {
      ...
      if (!password) {
        return res.status(400).send({
          message:
            "Não foi possível efetuar o login. A senha não foi inserida!",
          hint: "Por favor, insira uma senha.",
        });
      } else if (!email) {
        return res.status(400).send({
          message:
            "Não foi possível efetuar o login. O email não foi inserido!",
      ...
      loginUserUseCase.execute(email, (err, user) => {
        if (err?.name == "User not found") {
          return res.status(400).send({
            message:
              "Não foi possível efetuar o login. Este email não existe no sistema!",
            details: err.message,
